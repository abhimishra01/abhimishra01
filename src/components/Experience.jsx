    ...
    <section id="experience" className="py-24 px-6 bg-[#111111]">
      <div className="max-w-6xl mx-auto">
        ...
        <motion.div
          ...
          className="mb-14"
        >
          <h2 className="text-3xl font-bold text-slate-100 mb-2">My Journey</h2>
                ...
                >
                  ...
                  <div
                    className={`absolute left-0 top-1 w-8 h-8 rounded-full border-2 flex items-center justify-center ${
                      isWork
                        ? 'bg-orange-500 border-orange-500/40'
                  ...
                  {/* Card */}
                  <div className="rounded-xl border border-slate-800 bg-[#0a0a0a] p-6 hover:border-slate-700 transition-colors">
                    {/* Header */}
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1 mb-1">
                      <div>
                        <h3 className="text-base font-bold text-slate-100">{item.title}</h3>
                        <p className={`text-sm font-semibold ${isWork ? 'text-orange-400' : 'text-indigo-400'}`}>
                          {item.company}
                        </p>
                    ...
                    <ul className="mt-4 flex flex-col gap-2">
                      {item.highlights.map((point, j) => (
                        <li key={j} className="flex items-start gap-2.5 text-sm text-slate-400 leading-relaxed">
                          <span className={`mt-1.5 shrink-0 w-1.5 h-1.5 rounded-full ${isWork ? 'bg-orange-500' : 'bg-indigo-500'}`} />
                          {point}
                        </li>
                      ))}
                    </ul>
